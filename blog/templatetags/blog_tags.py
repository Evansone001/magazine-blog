...
def load_sidebar(user, linktype):
    ...
    if value:
        ...
        dates = Article.objects.datetimes(
        ...
        # Calculate the average value based on the total number, the size is (number/average)*step size
        increment = 5
        ...
        logger.info('set sidebar cache.key:{key}'.format(
            key="sidebar" + linktype))
        value['user'] = user
        return value
...
def load_pagination_info(page_obj, page_type, tag_name):
    ...
    if page_type == 'Classification Label Archive':
        ...
        if page_obj.has_previous():
            ...
            previous_url = reverse(
                ...
                kwargs={
                    'page': previous_number,
                    'tag_name': tag.slug})
    if page_type == 'Author Articles Archive':
        if page_obj.has_next():
            next_number = page_obj.next_page_number()
            next_url = reverse(
...
def load_article_detail(article, isindex, user):
    """
    Categories Load article details
    :param article:
    :param isindex: Whether it is a list page, if it is a list page, only the summary is displayed
    :return:
    """
    from djangoblog.utils import get_blog_setting
    blogsetting = get_blog_setting()
