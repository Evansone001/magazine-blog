...
class ArticleTest(TestCase):
    ...
    def test_validate_article(self):
        ...
        self.__check_pagination__(p, 'ClassificationLabel归档', tag.slug)

        p = Paginator(
            Article.objects.filter(
                author__username='liangliangyy'), 2)
        self.__check_pagination__(p, 'Author Articles Archive', 'liangliangyy')

        p = Paginator(Article.objects.filter(category=category), 2)
        self.__check_pagination__(p, 'Categories file', category.slug)
